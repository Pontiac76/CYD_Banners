...
- Current firmware in `src/main.cpp` initializes TFT, touch, LittleFS, SD, parses `SD://Banners/playlist.txt`, expands `PLAYLIST:` includes into an in-RAM slide list, renders basic `TEXT:` slides with `1:`/`2:` heading markers, renders `QR:` slides, accepts `IMAGE:`/`IMG:` entries but currently shows only an image-renderer placeholder for existing image files, and skips missing image/files while tracking a rolling missing-file list for the touch info screen. Full-screen sprite allocation failed on current hardware/runtime, so current flicker control uses direct full-screen draws only on slide/screen changes plus incremental progress bar drawing.
