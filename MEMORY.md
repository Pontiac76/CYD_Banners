...
- Firmware update flow: poll remote `sum.txt` roughly once per minute; fetch `manifest.txt` only when `sum.txt` changes or local state reset/deleted forces it. Manifest planning loads the RAM manifest, checks priority/current-playlist files visibly and blocking as needed, downloads playlist files first, rebuilds the runtime playlist, downloads priority/current-display files, rebuilds the runtime playlist again, then queues non-priority/background downloads. Non-priority/background manifest checks do not MD5/scan SD in the blocking phase; they rely on local `SD://banners/manifest.txt` comparison and queue future-job changes for background download. Background downloads intentionally do not show the large on-screen download panel; call-home/sum checks are suppressed while background downloads are pending. Local `manifest.txt`/`sum.txt` are written once priority/current-display content is healthy even if background downloads remain queued, preventing reset-induced infinite redownload loops.
